...
class WebInfo:
	...
	def _speedtest_cli_available(self):
		return shutil.which('speedtest') is not None or shutil.which('speedtest.exe') is not None

	def _run_speedtest_cli(self, timeout=30):
		try:
			cmd = shutil.which('speedtest') or shutil.which('speedtest.exe')
			if not cmd:
				return None
			proc = subprocess.run([cmd, "--format=json"], capture_output=True, text=True, timeout=timeout)
			if proc.returncode != 0:
				return None
			import json
			data = json.loads(proc.stdout)
			res = {
				"download_speed": round((data.get('download', {}).get('bandwidth', 0) * 8) / 1_000_000, 2),
				"upload_speed": round((data.get('upload', {}).get('bandwidth', 0) * 8) / 1_000_000, 2),
				"ping": round(data.get('ping', {}).get('latency', 0.0), 2),
				"server_info": {
					"name": data.get('server', {}).get('name'),
					"country": data.get('server', {}).get('country'),
					"sponsor": data.get('server', {}).get('host')
				}
			}
			return res
		except Exception:
	...
	def get_speed_test_info(self, max_attempts=3):
		...
		for attempt in range(max_attempts):
			try:
				...
				self.download_speed = round(download_bps / 1_000_000, 2)
				print("   Yükleme hızı ölçülüyor...")
				upload_bps = st.upload()
				self.upload_speed = round(upload_bps / 1_000_000, 2)
				...
				if attempt < max_attempts - 1:
					...
					print("❌ Python speedtest başarısız oldu!")
					break

		# Fallback: CLI mevcutsa onu dene
		if self._speedtest_cli_available():
			print("⚡ Speedtest CLI ile tekrar deneniyor...")
			cli_res = self._run_speedtest_cli()
			if cli_res:
				self.download_speed = cli_res.get("download_speed")
				self.upload_speed = cli_res.get("upload_speed")
				self.ping = cli_res.get("ping")
				self.server_info = cli_res.get("server_info")
				self.last_speed_test = datetime.now().isoformat()
				print(f"✅ CLI hız testi tamamlandı: ↓{self.download_speed} Mbps ↑{self.upload_speed} Mbps 🏓{self.ping} ms")
				return {
					"download_speed": self.download_speed,
					"upload_speed": self.upload_speed,
					"ping": self.ping,
		...
		print("❌ Hız testi tamamen başarısız oldu, bu metrik atlanacak.")
		return None
